...
character_y_pos = screen_height - character_height # 화면 세로 크기의 가장 아해에 해당하는 곳에 위치(세로)
...
# 적 enemy 캐릭터
enemy = pygame.image.load("C:\\Users\\user\\PycharmProjects\\pythonProject\\pygame_basic\\enemy.png")
...
enemy_y_pos = (screen_height/2) - (enemy_height/2) # 화면 세로 크기의 가장 아해에 해당하는
...
# 20 fps : 1초 동안에 20번 동작 -> 1번에 몇 만큼 이동? 5만큼 : 5 * 20 = 100
    ...
    character_x_pos += to_x * dt # 프레임을 증가시키더라도 게임 속도 유지
    character_y_pos += to_y * dt # 프레임을 증가시키더라도 게임 속도 유지
    ...
    # screen.fill((0,0,225)) #RGB로 색깔 채우기
